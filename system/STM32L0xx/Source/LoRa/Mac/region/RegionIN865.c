 ...
 * LoRaMAC channels
 */
extern ChannelParams_t LoRaMacChannels[IN865_MAX_NB_CHANNELS];

/*!
 * LoRaMac bands
 */
extern Band_t LoRaMacBands[IN865_MAX_NB_BANDS];
...
static void RegionIN865InitDefaults( InitType_t type )
{
    switch( type )
    {
        ...
        {
	    // Bands
	    LoRaMacBands[0] = ( Band_t ) IN865_BAND0;

            // Channels
            LoRaMacChannels[0] = ( ChannelParams_t ) IN865_LC1;
            LoRaMacChannels[1] = ( ChannelParams_t ) IN865_LC2;
            LoRaMacChannels[2] = ( ChannelParams_t ) IN865_LC3;
...
static bool RegionIN865NextChannel( NextChanParams_t* nextChanParams, uint8_t* channel, TimerTime_t* time, TimerTime_t* aggregatedTimeOff )
{
    ...
    if( nextChanParams->AggrTimeOff <= TimerGetElapsedTime( nextChanParams->LastAggrTx ) )
    {
        ...
        *aggregatedTimeOff = 0;
        ...
        nextTxDelay = RegionCommonUpdateBandTimeOff( nextChanParams->Joined, nextChanParams->DutyCycleEnabled, LoRaMacBands, IN865_MAX_NB_BANDS );
...
static bool RegionIN865ChannelsRemove( ChannelRemoveParams_t* channelRemove  )
{
    ...
    return RegionCommonChanDisable( LoRaMacChannelsMask, id, IN865_MAX_NB_CHANNELS );
}
...
const LoRaMacRegion_t LoRaMacRegionIN865 = {
