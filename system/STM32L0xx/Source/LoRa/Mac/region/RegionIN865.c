 ...
 * LoRaMAC channels
 */
extern ChannelParams_t RegionChannels[IN865_MAX_NB_CHANNELS];

/*!
 * LoRaMac bands
 */
extern Band_t RegionBands[IN865_MAX_NB_BANDS];
...
void RegionIN865InitDefaults( InitType_t type )
{
    switch( type )
    {
        ...
        {
            // Channels
            RegionChannels[0] = ( ChannelParams_t ) IN865_LC1;
...
void RegionIN865ApplyCFList( ApplyCFListParams_t* applyCFList )
{
    ...
    for( uint8_t i = 0, chanIdx = IN865_NUMB_DEFAULT_CHANNELS; chanIdx < IN865_MAX_NB_CHANNELS; i+=3, chanIdx++ )
    {
        ...
        else
        {
            ...
            RegionIN865ChannelRemove( &channelRemove );
        }
    }
}
...
uint8_t RegionIN865NewChannelReq( NewChannelReqParams_t* newChannelReq )
{
    ...
    if( newChannelReq->NewChannel->Frequency == 0 )
    {
        ...
        // Remove
        if( RegionIN865ChannelRemove( &channelRemove ) == false )
        {
            status &= 0xFC;
        }
    }
...
bool RegionIN865ChannelRemove( ChannelRemoveParams_t* channelRemove  )
{
    ...
    return RegionCommonChanDisable( RegionChannelsMask, id, IN865_MAX_NB_CHANNELS );
}
