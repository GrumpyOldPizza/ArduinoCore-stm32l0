 ...
 * LoRaMAC channels
 */
extern ChannelParams_t LoRaMacChannels[KR920_MAX_NB_CHANNELS];

/*!
 * LoRaMac bands
 */
extern Band_t LoRaMacBands[KR920_MAX_NB_BANDS];
...
static PhyParam_t RegionKR920GetPhyParam( GetPhyParams_t* getPhy )
{
...
static void RegionKR920SetBandTxDone( SetBandTxDoneParams_t* txDone )
{
    RegionCommonSetBandTxDone( txDone->Joined, &LoRaMacBands[LoRaMacChannels[txDone->Channel].Band], txDone->LastTxDoneTime );
}

static void RegionKR920InitDefaults( InitType_t type )
{
    switch( type )
    {
        case INIT_TYPE_INIT:
        {
	    // Bands
	    LoRaMacBands[0] = ( Band_t) KR920_BAND0;

            // Channels
            LoRaMacChannels[0] = ( ChannelParams_t ) KR920_LC1;
            LoRaMacChannels[1] = ( ChannelParams_t ) KR920_LC2;
            LoRaMacChannels[2] = ( ChannelParams_t ) KR920_LC3;
...
static bool RegionKR920Verify( VerifyParams_t* verify, PhyAttribute_t phyAttribute )
{
    switch( phyAttribute )
    {
        case PHY_TX_DR:
...
static void RegionKR920ApplyCFList( ApplyCFListParams_t* applyCFList )
{
...
static bool RegionKR920ChanMaskSet( ChanMaskSetParams_t* chanMaskSet )
{
...
static bool RegionKR920AdrNext( AdrNextParams_t* adrNext, int8_t* drOut, int8_t* txPowOut, uint32_t* adrAckCounter )
{
...
static void RegionKR920ComputeRxWindowParameters( int8_t datarate, uint8_t minRxSymbols, uint32_t rxError, RxConfigParams_t *rxConfigParams )
{
    double tSymbol = 0.0;
...
static bool RegionKR920RxConfig( RxConfigParams_t* rxConfig, int8_t* datarate )
{
...
static bool RegionKR920TxConfig( TxConfigParams_t* txConfig, int8_t* txPower, TimerTime_t* txTimeOnAir )
{
    ...
    float maxEIRP = GetMaxEIRP( LoRaMacChannels[txConfig->Channel].Frequency );
    int8_t phyTxPower = 0;
...
static uint8_t RegionKR920LinkAdrReq( LinkAdrReqParams_t* linkAdrReq, int8_t* drOut, int8_t* txPowOut, uint8_t* nbRepOut, uint8_t* nbBytesParsed )
{
    uint8_t status = 0x07;
...
static uint8_t RegionKR920RxParamSetupReq( RxParamSetupReqParams_t* rxParamSetupReq )
{
    uint8_t status = 0x07;
...
static uint8_t RegionKR920NewChannelReq( NewChannelReqParams_t* newChannelReq )
{
    uint8_t status = 0x03;
    ChannelAddParams_t channelAdd;
    ChannelRemoveParams_t channelRemove;
...
static int8_t RegionKR920TxParamSetupReq( TxParamSetupReqParams_t* txParamSetupReq )
{
    return -1;
}

static uint8_t RegionKR920DlChannelReq( DlChannelReqParams_t* dlChannelReq )
{
    uint8_t status = 0x03;
...
static int8_t RegionKR920AlternateDr( AlternateDrParams_t* alternateDr )
{
    int8_t datarate = 0;
...
static void RegionKR920CalcBackOff( CalcBackOffParams_t* calcBackOff )
{
...
static bool RegionKR920NextChannel( NextChanParams_t* nextChanParams, uint8_t* channel, TimerTime_t* time, TimerTime_t* aggregatedTimeOff )
{
    ...
    if( nextChanParams->AggrTimeOff <= TimerGetElapsedTime( nextChanParams->LastAggrTx ) )
    {
        ...
        *aggregatedTimeOff = 0;
        ...
        nextTxDelay = RegionCommonUpdateBandTimeOff( nextChanParams->Joined, nextChanParams->DutyCycleEnabled, LoRaMacBands, KR920_MAX_NB_BANDS );
    ...
    if( nbEnabledChannels > 0 )
    {
        for( uint8_t  i = 0, j = randr( 0, nbEnabledChannels - 1 ); i < KR920_MAX_NB_CHANNELS; i++ )
        {
            ...
            if( Radio.IsChannelFree( MODEM_LORA, LoRaMacChannels[channelNext].Frequency, KR920_RSSI_FREE_TH, KR920_CARRIER_SENSE_TIME ) == true )
            {
                // Free channel found
                *channel = channelNext;
                *time = 0;
...
static LoRaMacStatus_t RegionKR920ChannelAdd( ChannelAddParams_t* channelAdd )
{
    uint8_t band = 0;
...
static bool RegionKR920ChannelsRemove( ChannelRemoveParams_t* channelRemove  )
{
    ...
    return RegionCommonChanDisable( LoRaMacChannelsMask, id, KR920_MAX_NB_CHANNELS );
}

static void RegionKR920SetContinuousWave( ContinuousWaveParams_t* continuousWave )
{
    ...
    float maxEIRP = GetMaxEIRP( LoRaMacChannels[continuousWave->Channel].Frequency );
    int8_t phyTxPower = 0;
...
static uint8_t RegionKR920ApplyDrOffset( uint8_t downlinkDwellTime, int8_t dr, int8_t drOffset )
{
