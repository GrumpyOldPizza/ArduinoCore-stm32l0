 ...
 * LoRaMAC channels
 */
extern ChannelParams_t LoRaMacChannels[CN470_MAX_NB_CHANNELS];

/*!
 * LoRaMac bands
 */
extern Band_t LoRaMacBands[CN470_MAX_NB_BANDS];
...
static void RegionCN470InitDefaults( InitType_t type )
{
    switch( type )
    {
        ...
        {
	    // Bands
	    LoRaMacBands[0] = ( Band_t )CN470_BAND0;

            // Channels
            // 125 kHz channels
            for( uint8_t i = 0; i < CN470_MAX_NB_CHANNELS; i++ )
            {
                LoRaMacChannels[i].Frequency = 470300000 + i * 200000;
            ...
            LoRaMacChannelsDefaultMask[4] = 0xFFFF;
            LoRaMacChannelsDefaultMask[5] = 0xFFFF;
                ...
                if( ( adrNext->AdrAckCounter % CN470_ADR_ACK_DELAY ) == 1 )
                {
                    ...
                    if( datarate == CN470_TX_MIN_DATARATE )
                    {
                        ...
                        if( adrNext->UpdateChanMask == true )
                        {
                            ...
                            LoRaMacChannelsMask[0] = 0xFFFF;
                            LoRaMacChannelsMask[1] = 0xFFFF;
                            LoRaMacChannelsMask[2] = 0xFFFF;
                            LoRaMacChannelsMask[3] = 0xFFFF;
                            LoRaMacChannelsMask[4] = 0xFFFF;
                            LoRaMacChannelsMask[5] = 0xFFFF;
                        }
                    }
                }
            }
        ...
        else
        {
            for( uint8_t i = 0; i < 16; i++ )
            {
                if( ( ( linkAdrParams.ChMask & ( 1 << i ) ) != 0 ) &&
                    ( LoRaMacChannels[linkAdrParams.ChMaskCtrl * 16 + i].Frequency == 0 ) )
...
static bool RegionCN470NextChannel( NextChanParams_t* nextChanParams, uint8_t* channel, TimerTime_t* time, TimerTime_t* aggregatedTimeOff )
{
    ...
    if( RegionCommonCountChannels( LoRaMacChannelsMask, 0, 6 ) == 0 )
    ...
    if( nextChanParams->AggrTimeOff <= TimerGetElapsedTime( nextChanParams->LastAggrTx ) )
    {
        ...
        *aggregatedTimeOff = 0;
        ...
        nextTxDelay = RegionCommonUpdateBandTimeOff( nextChanParams->Joined, nextChanParams->DutyCycleEnabled, LoRaMacBands, CN470_MAX_NB_BANDS );
