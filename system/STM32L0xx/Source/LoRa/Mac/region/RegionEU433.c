 ...
 * LoRaMAC channels
 */
extern ChannelParams_t RegionChannels[EU433_MAX_NB_CHANNELS];

/*!
 * LoRaMac bands
 */
extern Band_t RegionBands[EU433_MAX_NB_BANDS];
...
void RegionEU433InitDefaults( InitType_t type )
{
    switch( type )
    {
        ...
        {
            // Channels
            RegionChannels[0] = ( ChannelParams_t ) EU433_LC1;
...
void RegionEU433ApplyCFList( ApplyCFListParams_t* applyCFList )
{
    ...
    for( uint8_t i = 0, chanIdx = EU433_NUMB_DEFAULT_CHANNELS; chanIdx < EU433_MAX_NB_CHANNELS; i+=3, chanIdx++ )
    {
        ...
        else
        {
            ...
            RegionEU433ChannelRemove( &channelRemove );
        }
    }
}
...
uint8_t RegionEU433NewChannelReq( NewChannelReqParams_t* newChannelReq )
{
    ...
    if( newChannelReq->NewChannel->Frequency == 0 )
    {
        ...
        // Remove
        if( RegionEU433ChannelRemove( &channelRemove ) == false )
        {
            status &= 0xFC;
        }
    }
...
bool RegionEU433ChannelRemove( ChannelRemoveParams_t* channelRemove  )
{
    ...
    return RegionCommonChanDisable( RegionChannelsMask, id, EU433_MAX_NB_CHANNELS );
}
