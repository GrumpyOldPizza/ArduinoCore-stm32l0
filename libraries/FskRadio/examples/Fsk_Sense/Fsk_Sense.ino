/* Simple Ping-Pong with carrier sense for a FSK Radio/Modem
 *
...
void loop( void )
{
    ...
    case STATE_SCANNING:
            ...
            (FskRadio.read() == 'G'))
        {
            ...
            while (!FskRadio.sense(RSSI_THRESHOLD, SENSE_TIME)) {
                delay(50);
            }
    ...
    case STATE_RX_MASTER:
        if ((FskRadio.parsePacket() == 4) &&
            ...
            (FskRadio.read() == 'G'))
        {
            // Got a PONG from a slave, so send a PING as reply
